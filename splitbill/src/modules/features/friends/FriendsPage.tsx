...
interface IFriendsPageHeaderProps {
    setSearchQuery: Dispatch<SetStateAction<string>>;
}

const FriendsPageHeader = ({ setSearchQuery }: IFriendsPageHeaderProps) => {
    ...
    return (
        <div className='flex flex-col gap-4 h-28'>
            <div className='flex flex-row justify-between items-center'>
                ...
                >
                    <IoPersonAddSharp size={30} className='text-brand-orange' />
                </button>
            </div>
...
const FriendsPageBody = ({ searchQuery }: { searchQuery: string }) => {
    ...
    if (getFriendRequests.isError || getFriends.isError) {
        return (
            <div className='text-font-white text-lg font-semibold'>
                Error loading data
            </div>
        );
    }
        ...
        <div className='flex flex-col gap-4 overflow-y-scroll h-full pb-20 w-full'>
            ...
            {favouritedFriends.length > 0 && (
                <>
                    ...
                    <SwipeableList className='w-full' type={Type.IOS}>
                        {favouritedFriends?.map((friend) => (
                            <SwipeableListItem
                                ...
                                trailingActions={trailingActions(
                                    ...
                                    toggleDrawer,
                                    setFriendId
                                )} // Pass friend.id to identify which friend to delete
                                className='w-full '
                            >
                                <FriendCard
                                    key={friend.id}
